...
CampusPay is a synthetic education-finance workspace for tracking tuition payment initiations across
students, partner universities, lenders, and payment operations.
...
records and accounts are demo data. **No real money moves.** The repository still includes the
NestJS/domain/XRPL packages from the broader TuitionFlow platform, but the deployed CampusPay web
experience uses Firestore as its active data path.
